...
// 1. PERFORMANCE: Moved static data OUTSIDE the component.
// This prevents JavaScript from rebuilding these arrays every 2.5 seconds during the interval re-render.
...
const calculators = [
  ...
  },
  {
    ...
    description: "Get an estimate for custom-built, luxury storage solutions.",
    icon: (
      <svg
        aria-hidden="true"
        className="w-8 h-8"
  ...
  return (
    ...
    >
      ...
      <div className="w-full bg-[#2A3439] py-4 shadow-[0_10px_30px_rgba(0,0,0,0.2)] z-10 relative">
        <div className="max-w-7xl mx-auto px-4 text-center group cursor-pointer">
          <p className="text-[#C5A059] font-serif italic text-sm md:text-base tracking-[0.2em] transition-all duration-700 group-hover:tracking-[0.3em] group-hover:text-white flex items-center justify-center gap-4">
            <span className="h-px w-8 bg-[#C5A059] opacity-50 transition-all duration-700 group-hover:w-16"></span>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 w-full text-center">
        {/* 2. SEO: Screen-reader & Search Engine ONLY heading */}
        <h2 className="sr-only">
          Get the estimate for your Full Home, Kitchen, Wardrobe, or Office
          interiors in Warangal.
        </h2>

        {/* 3. ACCESSIBILITY: Hide the visual-only animated text from crawlers so they don't read gibberish */}
        <div
          aria-hidden="true"
          className="text-3xl md:text-5xl font-serif text-[#13503B] mb-4 font-bold flex flex-wrap items-center justify-center gap-x-3 gap-y-1 md:gap-y-2"
        >
          <span>Get the estimate for your</span>
          <span className="inline-block h-[1.2em] overflow-hidden align-bottom text-[#C5A059] min-w-50 md:min-w-70 text-center md:text-left">
            <span
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {calculators.map((calc, idx) => (
            ...
            >
              ...
              <p className="text-gray-500 text-sm mb-8 grow leading-relaxed">
                {calc.description}
              </p>

              {/* 4. SEO: Converted <button> to an <a> tag so Google can crawl your calculator paths */}
              <a
                href={calc.path}
                target="_blank"
                rel="noopener noreferrer"
                className="block w-full text-center relative overflow-hidden bg-gray-50 text-[#13503B] border border-gray-200 py-3.5 rounded font-semibold text-xs tracking-[0.2em] uppercase transition-all duration-500 group-hover:bg-[#13503B] group-hover:text-white group-hover:border-[#13503B]"
              >
                ...
                <span className="absolute inset-0 bg-linear-to-r from-transparent via-[#C5A059]/20 to-transparent -translate-x-full transition-transform duration-1000 ease-in-out group-hover:translate-x-full z-0"></span>
              </a>
            </div>
          ))}
        </div>
