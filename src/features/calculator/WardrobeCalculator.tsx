...
const TYPES = [
  { id: "Swing Door", name: "Swing Door", desc: "Classic hinged doors offering complete access to the cabinet.", image: swingDoorImg },
  { id: "Sliding Door", name: "Sliding Door", desc: "Sleek sliding mechanism, space-saving for modern bedrooms.", image: slideDoorImg },
  { id: "Walk-in Wardrobe", name: "Walk-in Wardrobe", desc: "Open-plan luxury dressing space with partition layouts.", image: walkInWardrobeImg }
];

const FINISHES = [
  { id: "Laminate", name: "Laminate", desc: "Matte or glossy protective overlays, scratch-resistant, daily-use robust.", tag: "Standard", image: laminateImg },
  { id: "Acrylic", name: "Acrylic", desc: "Mirror-like ultra-glossy finish, stain-resistant premium visuals.", tag: "Premium", image: acrylicImg },
  { id: "Glass", name: "Glass", desc: "Tinted transparent or reflective backing, high-end reflection aesthetic.", tag: "Premium", image: glassImg },
  { id: "PU Finish", name: "PU Finish", desc: "Premium seamless spray paint finish, customizable colors, luxurious silk feel.", tag: "Luxury", image: puFinishImg },
  { id: "Veneer", name: "Veneer", desc: "Natural wood sheet overlay, polished for an authentic, warm premium look.", tag: "Luxury", image: veneerImg }
];

const MATERIALS = [
  { id: "Plywood", name: "Plywood", desc: "High tensile strength, water-resistant core, extremely durable.", badge: "Excellent Durability", image: PlywoodImg },
  { id: "HDHMR", name: "HDHMR", desc: "High Density High Moisture Resistant, superior termite and water resistance.", badge: "Highest Durability", image: HDHMRImg },
  { id: "MDF", name: "MDF", desc: "Medium Density Fiberboard, ultra-smooth surface, ideal for clean finishes.", badge: "Moderate Durability", image: MDFImg },
  { id: "Particle Board", name: "Particle Board", desc: "Eco-friendly, lightweight wood panels, best for light usage.", badge: "Standard Durability", image: ParticleBoardImg }
];

const ACCESSORIES = [
  { id: "Drawers", name: "Drawers", desc: "Convenient pull-out drawers for folded clothing.", image: drawersImg },
  { id: "Shoe Rack", name: "Shoe Rack", desc: "Dedicated shelving to keep your footwear organized.", image: shoeRackImg },
  { id: "Jewellery Organizer", name: "Jewellery Organizer", desc: "Velvet-lined compartment for accessories.", image: jewelleryOrganizerImg },
  { id: "Mirror", name: "Mirror", desc: "Integrated full-length mirror for styling.", image: mirrorImg },
  { id: "LED Lighting", name: "LED Lighting", desc: "Warm sensored lighting inside the wardrobe.", image: ledLightingImg },
  { id: "Soft Close Hinges", name: "Soft Close Hinges", desc: "Hinges that prevent slamming and close silently.", image: softCloseHingesImg },
  { id: "Tie Holder", name: "Tie Holder", desc: "Dedicated slide-out holder for ties and belts.", image: tieHolderImg },
  { id: "Trouser Pull-Out", name: "Trouser Pull-Out", desc: "Crease-free multi-tier trouser hanging rack.", image: trouserPullOutImg },
  { id: "Loft Storage", name: "Loft Storage", desc: "Top-level cabinets for suitcases and seasonal items.", image: walkInWardrobeImg },
  { id: "Hanging Rod", name: "Hanging Rod", desc: "Sturdy rod for hanging shirts, coats, and dresses.", image: swingDoorImg }
];
...
const CalculatorDropdown: React.FC<CalculatorDropdownProps> = ({ value, options, onChange }) => {
...
export const WardrobeCalculator: React.FC = () => {
  ...
  const submitToBackend = async (customerDetails: CustomerDetails) => {
    const displayLength = selectedLength.includes("ft") ? selectedLength : `${selectedLength} ft`;
    const displayHeight = selectedHeight === "Ceiling Height" ? "Ceiling Height" : (selectedHeight.includes("ft") ? selectedHeight : `${selectedHeight} ft`);
    const accessoriesText = selectedAccessories.length > 0 ? selectedAccessories.join(", ") : "None";
      ...
      <div className="max-w-3xl w-full mx-auto bg-white rounded-2xl shadow-[0_20px_60px_rgba(0,0,0,0.05)] overflow-hidden border border-gray-100 flex flex-col min-h-[600px]">
        ...
        <div className="flex-1 p-8 sm:p-10 flex flex-col justify-center">
          ...
          {currentStep === 0 && (
            <div className="animate-fadeIn text-center">
              <span className="text-[#C5A059] font-black text-xs uppercase tracking-widest">Step 1 of 5</span>
              <h2 className="text-2xl sm:text-3xl font-serif text-[#13503B] font-bold mt-2 mb-4">Choose your wardrobe type</h2>
                ...
                {TYPES.map((t) => (
                  <div key={t.id} className="flex flex-col text-left group">
                    <button
                      ...
                      className={`relative rounded-2xl border-2 overflow-hidden transition-all duration-300 h-48 w-full cursor-pointer ${selectedType === t.id
                    ...
                    >
                      ...
                      <span className="absolute bottom-3 left-4 z-20 text-base font-bold text-white tracking-wide">{t.name}</span>
                      {selectedType === t.id && (
          ...
          {currentStep === 1 && (
            <div className="animate-fadeIn text-center flex flex-col items-center">
              <div className="text-center mb-8 w-full">
                <span className="text-[#C5A059] font-black text-xs uppercase tracking-widest">Step 2 of 5</span>
                <h2 className="text-2xl sm:text-3xl font-serif text-[#13503B] font-bold mt-2 mb-2">Specify wardrobe dimensions</h2>
                <p className="text-gray-500 text-sm">Select the length and height measurements for your wardrobe cabinet layout.</p>
              </div>
              ...
              <div className="w-full max-w-md space-y-5 bg-white border border-gray-100 rounded-2xl p-6 shadow-[0_4px_20px_rgba(0,0,0,0.02)] mb-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-base font-bold text-gray-550 w-6 text-right">ft</span>
                  </div>
                  <p className="text-gray-400 text-[10px] text-left mt-1.5 pl-1">Outer length (horizontal width) of cabinet layout.</p>
                </div>
                ...
                <div>
                  ...
                  <p className="text-gray-400 text-[10px] text-left mt-1.5 pl-1">Height of your wardrobe cabinet layout.</p>
                </div>
              </div>
            </div>
          )}
          ...
          {currentStep === 2 && (
            <div className="animate-fadeIn text-center">
              <span className="text-[#C5A059] font-black text-xs uppercase tracking-widest">Step 3 of 5</span>
              <h2 className="text-2xl sm:text-3xl font-serif text-[#13503B] font-bold mt-2 mb-4">Choose your preferred finish</h2>
              ...
              <div className="grid grid-cols-2 gap-3 sm:gap-4 mb-8">
                {FINISHES.map((f) => (
                  <button
                    ...
                    className={`relative rounded-2xl border-2 overflow-hidden transition-all duration-300 min-h-[160px] sm:min-h-[240px] text-left group cursor-pointer ${selectedFinish === f.id
                      ? "border-[#C5A059] shadow-lg scale-[1.02]"
                      : "border-gray-100 hover:border-[#13503B] hover:shadow-md hover:scale-[1.01]"
                      }`}
                  >
                    ...
                    />
                    <div className={`absolute inset-0 transition-all duration-300 z-10 ${selectedFinish === f.id
                    ...
                    <div className="relative z-20 p-2.5 sm:p-5 h-full flex flex-col justify-end min-h-[160px] sm:min-h-[240px] pointer-events-none">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        <span className="text-xs sm:text-lg font-bold text-white tracking-wide">{f.name}</span>
          ...
          {currentStep === 3 && (
            <div className="animate-fadeIn text-center">
              <span className="text-[#C5A059] font-black text-xs uppercase tracking-widest">Step 4 of 5</span>
              <h2 className="text-2xl sm:text-3xl font-serif text-[#13503B] font-bold mt-2 mb-4">Choose the core material</h2>
              ...
              <div className="grid grid-cols-2 gap-3 sm:gap-4 mb-8">
                {MATERIALS.map((mat) => (
                  <button
                    ...
                    className={`relative rounded-2xl border-2 overflow-hidden transition-all duration-300 min-h-[160px] sm:min-h-[240px] text-left group cursor-pointer ${selectedMaterial === mat.id
                      ? "border-[#C5A059] shadow-lg scale-[1.02]"
                      : "border-gray-100 hover:border-[#13503B] hover:shadow-md hover:scale-[1.01]"
                      }`}
                  >
                    <img
                      ...
                      className="absolute inset-0 w-full h-full object-cover transition-transform duration-500 group-hover:scale-110"
                    />
                    <div className={`absolute inset-0 transition-all duration-300 z-10 ${selectedMaterial === mat.id
                      ? "bg-gradient-to-t from-[#13503B]/95 via-[#13503B]/60 to-[#13503B]/20"
                      : "bg-gradient-to-t from-black/85 via-black/45 to-transparent group-hover:from-black/90 group-hover:via-black/55"
                      }`} />
                    ...
                    <div className="relative z-20 p-2.5 sm:p-5 h-full flex flex-col justify-end min-h-[160px] sm:min-h-[240px] pointer-events-none">
                      <span className="block text-xs sm:text-lg font-bold text-white tracking-wide">{mat.name}</span>
                      <span className="block text-[9px] sm:text-xs text-gray-200 mt-0.5 sm:mt-2 line-clamp-2 leading-relaxed">
                        {mat.desc}
                      </span>
                    </div>
          ...
          {currentStep === 4 && (
            <div className="animate-fadeIn text-center">
              <span className="text-[#C5A059] font-black text-xs uppercase tracking-widest">Step 5 of 5</span>
              <h2 className="text-2xl sm:text-3xl font-serif text-[#13503B] font-bold mt-2 mb-4">Select accessories</h2>
                ...
                {ACCESSORIES.map((acc) => {
                  ...
                  return (
                    <button
                      ...
                      className={`relative rounded-xl border-2 transition-all duration-300 flex items-center text-left cursor-pointer overflow-hidden ${isChecked
                    ...
                    >
                      ...
                      <div className="flex-1 p-3 pr-8 min-w-0">
                        <span className="font-bold text-gray-800 text-sm block truncate">{acc.name}</span>
                        <p className="text-[11px] text-gray-400 mt-1 leading-tight line-clamp-2">{acc.desc}</p>
                      </div>
          ...
          {currentStep === 5 && (
            <div className="animate-fadeIn text-center">
              ...
              <h2 className="text-2xl sm:text-3xl font-serif text-[#13503B] font-bold mt-2 mb-4">Configurator Summary</h2>
                ...
                <div className="flex items-center justify-between border-b border-gray-200/50 pb-3">
                  <div>
                    ...
                    <span className="text-sm font-bold text-gray-800 font-sans">
                      {selectedLength.includes("ft") ? selectedLength : `${selectedLength} ft`}
                    </span>
                  </div>
                ...
                <div className="flex items-center justify-between border-b border-gray-200/50 pb-3">
                  <div>
                    ...
                    <span className="text-sm font-bold text-gray-800">
                      {selectedHeight === "Ceiling Height" ? "Ceiling Height" : (selectedHeight.includes("ft") ? selectedHeight : `${selectedHeight} ft`)}
                    </span>
                  </div>
                  <button onClick={() => setCurrentStep(1)} className="text-[#13503B] hover:text-[#0d3528] transition-colors">
                    <Edit2 size={16} />
        ...
        {currentStep >= 0 && currentStep < 7 && (
          <div className="px-4 py-4 sm:px-8 sm:py-6 bg-gray-50 border-t border-gray-100 flex justify-between items-center mt-auto">
            ...
            >
              <ArrowLeft size={12} className="transition-transform group-hover:-translate-x-0.5 sm:w-3.5 sm:h-3.5" />
              GO BACK
            </button>
            ...
            {(currentStep >= 0 && currentStep <= 5) && (
              ...
              >
                CONTINUE
                <ChevronRight size={12} className="transition-transform group-hover:translate-x-0.5 sm:w-3.5 sm:h-3.5" />
              </button>
            )}
          </div>
        )}
